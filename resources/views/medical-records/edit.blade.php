            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="mb-6">
                        ...
                        <p class="mt-1 text-sm text-gray-600">Name: {{ $medicalRecord->appointment->patient->user->name }}</p>
                        <p class="mt-1 text-sm text-gray-600">Appointment Date: {{ $medicalRecord->appointment->date->format('d M Y') }}</p>
                        <p class="mt-1 text-sm text-gray-600">Complaint: {{ $medicalRecord->appointment->complaint }}</p>
                    </div>

                    <form method="POST" action="{{ route('medical-records.update', $medicalRecord->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="diagnosis" name="diagnosis" rows="3" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>{{ old('diagnosis', $medicalRecord->diagnosis) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="doctor_notes" name="doctor_notes" rows="4" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('doctor_notes', $medicalRecord->doctor_notes) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            @if($medicalRecord->attachment_path)
                                <div class="mb-2 text-sm text-gray-600">
                                    Current file: <a href="{{ asset('storage/' . $medicalRecord->attachment_path) }}" target="_blank" class="text-indigo-600 hover:underline">View Attachment</a>
                                </div>
                            @endif
                            ...
                            <p class="mt-1 text-sm text-gray-500">PDF, JPG, or PNG (Max. 2MB). Uploading a new file will replace the existing one.</p>
                            <x-input-error :messages="$errors->get('attachment')" class="mt-2" />
                        </div>

                        <!-- Prescriptions -->
                        <div class="mb-6" x-data="{ 
                            prescriptions: {{ json_encode($medicalRecord->prescriptions->map(function($p){ return ['medication' => $p->medication, 'dosage' => $p->dosage, 'instructions' => $p->instructions]; })) }} 
                        }">
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('medical-records.show', $medicalRecord->id) }}" class="text-gray-600 hover:text-gray-900 mr-4">Cancel</a>
                            <x-primary-button>
                                {{ __('Update Record') }}
                            </x-primary-button>
                        </div>
